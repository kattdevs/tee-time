...
import {createBooking, fetchSlots, selectCourse, selectDate, selectLoading, selectCourses} from '../store/bookingSlice';
...
export default function BookingModal () {
    ...
    const courses = useSelectors(selectCourses);

    //Full the full course name for display
    ...
    const validate = () => {
        ...
        else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) e.mail = 'Enter a valid email';
        return e;
    };
    ...
    const handleSubmit = async () => {
        ...
        const result = await dispatch(createBooking({
            ...
            if (createBooking.fulfilled.match(result)) {
                ...
                dispatch(fetchSlots({course, date: cleanDate})); //refresh available slots
            } else {
                dispatch(showToast({message:result.payload || 'Booking failed. Please try again', 
                    type:'error'
                }));
            }
            };
            const inputStyle = field => ({
                ...
                border: errors[field] ? '1px solid #EF4444' : '1px solid rgba(255,255,255,0.01)',
                ...
                return (
                    ...
                    style={{background:'rgba(13,59,30,0.97)', borderRadius:'24px', padding:'32px', width:'100%', maxWidth:'460px',border:'1px solid rgba(201,168,76,0.4)',boxShadow:'0 20px 60px rgba(0,0,0,0.4)', backdropFilter:'blur(20px)'}}
                    >
                        {/*Header*/}
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'24px'}}>
                            <div>
                                <h2 style={{fontFamily:'Playfair Display serif', color:'white', fontSize:'22px', fontWeight:'700', margin:0}}>Confirm Booking</h2>
                                <p style={{color:'#C9A84C', fontSize:'14px', marginTop:'6px'}}>{slot} &nbsp;&bull;&nbsp; {date} &nbsp;&bull;&nbsp; {courseName}</p>
                                </div>
                                <button onClick={() => dispatch(closeModal())} style={{background:'rgba(255,255,255,0.1)', border:'none',borderRadius:'50%', width:'34px', height: '34px', display:'flex', alignItems:'center',justifyContent:'center',cursor:'pointer', color:'white', flexShrink: 0}}>
                                    <X size={16} />
                                </button>
                            </div>
                            {/*Form*/}
